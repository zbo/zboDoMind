...
if __name__ == '__main__':
    ...
    print "涨停板数量大于50:{0} 得分{1}".format(ztsl, ztsl_score)
    # 连续涨停板数量 大于 10 +2分
    lxztsl = int(webget('非st连续涨停'))
    lxztsl_score = score(lxztsl > 10)
    print "连续涨停板数量大于10:{0} 得分{1}".format(lxztsl, lxztsl_score)
    # 跌幅大于5%不包含跌停 小于 100 +2分
    xd5 = int(webget('非st跌幅大于5%不包含跌停'))
    xd5_score = score(xd5 < 100)
    print "跌幅大于5%不包含跌停数量小于100:{0} 得分{1}".format(xd5, xd5_score)
    # 红盘个数比 大于一半 +2分
    sz = int(webget('上涨'))
    xd = int(webget('下跌'))
    hp_score = score(sz > xd)
    print "上涨数量:{0} 下跌数量:{1} 得分{2}".format(sz, xd, hp_score)
    # 下跌5%和涨停比较得分 3<1 +2分
    str_score = score(ztsl > xd5)
    print "涨停数量:{0} 下跌5%数量:{1} 得分{2}".format(ztsl, xd5, str_score)
    print "---------------"
