...
if __name__ == '__main__':
    ...
    ret_list = []
    # 昨日涨停今日红盘比 大于 0.6 +2分
    zrzt = int(webget("非st，周4涨停"))
    zrzt_hp = int(webget("非st，周4涨停, 周5上涨"))
    score1 = score(zrzt * 0.6 < zrzt_hp)
    print "[60%+]昨日涨停:{0} 今天仍然红盘:{1} 得分:{2}".format(zrzt, zrzt_hp, score1)

    # 昨日连续涨停今日红盘比 大于 0.6 +2分
    zrlxzt = int(webget("非st，周四连板"))
    zrlxzt_hp = int(webget("非st，周四连板, 今日上涨"))
    score2 = score(zrlxzt * 0.6 < zrlxzt_hp)
    print "[60%+]昨日连续涨停:{0} 今天仍然红盘:{1} 得分:{2}".format(zrlxzt, zrlxzt_hp, score2)

    # 昨日涨停今日跌幅超4%比例 小于 0.3 +2分
    zrzt_dm = int(webget("非st，周4涨停, 今日跌幅大于4%"))
    score3 = score(zrzt_dm < zrzt*0.3)
    print "[30%-]昨日涨停:{0} 今天大面:{1} 得分:{2}".format(zrzt, zrzt_dm, score3)

    # 昨日连板涨停今日跌幅超4%比例 小于 0.3 +2分
    zrlxzt_dm = int(webget("非st，周四连板, 今日跌幅大于4%"))
    score4 = score(zrlxzt_dm < zrlxzt * 0.3)
    print "[30%-]昨日连续涨停:{0} 今天大面:{1} 得分:{2}".format(zrlxzt, zrlxzt_dm, score4)

    # 昨日断板绿盘比例 小于 0.4 +2分
    lp = int(webget("非st，周4未能涨停，周3涨停"))
    lp2 = int(webget("非st，周4未能涨停，周3涨停, 今天下跌"))
    score5 = score(lp2 < lp * 0.4)
    print "[40%-]昨日断板:{0} 断板后今天下跌:{1} 得分:{2}".format(lp, lp2, score5)

    # 今日炸板和涨停比 小于 1/3 +2分
    zb = int(webget("非st，炸板"))
    zt = int(webget("非st，涨停"))
    score6 = score(zb*3 < zt)
    print "[30%-]炸板:{0} 涨停:{1} 得分:{2}".format(zb,zt,score6)
    print "---------------"
