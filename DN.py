...
if __name__ == '__main__':
    ...
    print "[昨日涨停大面40%-] 昨日涨停:{0} 今天大面:{1} 得分:{2}".format(zrzt, zrzt_dm, score3)

    # 昨日连板涨停今日跌幅超4%比例 小于 0.4 +2分
    zrlxzt_dm = int(webget("非st，非新股，昨日连板, 今日跌幅大于4%"))
    score4 = score(zrlxzt_dm < zrlxzt * 0.4)
    print "[昨日连板大面40%-] 昨日连续涨停:{0} 今天大面:{1} 得分:{2}".format(zrlxzt, zrlxzt_dm, score4)

    # 今日炸板和涨停比 小于 40% +2分
    zb = int(webget("非st，非新股，今日炸板"))
    zt = int(webget("非st，非新股，今日涨停"))
    score6 = score(zb < zt*0.4)
    print "[今日炸板40%-] 炸板:{0} 涨停:{1} 得分:{2}".format(zb,zt,score6)
    print "---------------"
